  ...
  Drupal.behaviors.termReferenceTree = {
    attach: function (context, settings) {
      ...
      termReferenceTree.forEach(function (tree) {
        ...
        if (tree.hasClass('term-reference-tree-select-parents')) {
        ...
        if (tree.hasClass('term-reference-tree-track-list-shown')) {
          ...
          var input_type =
            (tree.has('input[type=checkbox]').length > 0) ? 'checkbox' : 'radio';

          //Find all the checked controls.
          var checked_controls = tree.find('input[type=' + input_type + ']:checked');
          ...
          tree.find('.form-' + input_type).change(function (event) {
            //Remove the "nothing selected" message if showing - add it later
            // if needed.
