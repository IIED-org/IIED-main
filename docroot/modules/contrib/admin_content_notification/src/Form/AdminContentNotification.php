...
class AdminContentNotification extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, Request $request = NULL) {
    ...
    $form['admin_content_notification_recepient_fieldset']['admin_content_notification_default_to'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Default To Address'),
      '#default_value' => !empty($config->get('admin_content_notification_default_to')) ? $config->get('admin_content_notification_default_to') : $site_email,
      '#description' => $this->t('This email address will be used as default "to" address, as all the other recepients email ids will be passed as bcc to prevent full list of recipients to be disclosed.'),
    ];
    ...
    $form['admin_content_notification_recepient_fieldset']['admin_content_notification_email'] = [
      ...
      '#default_value' => isset($admin_content_notification_email) ? $admin_content_notification_email : '',
    ...
    if ($this->adminContentNotificationService->isTokenEnabled()) {
      $form['admin_content_notification_email_fieldset']['admin_content_notification_email_tokens']['#markup'] = '<strong>' . $this->t('You can use tokens provided by token module as well (node and user tokens only).') . '</strong><br>';
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config->set('admin_content_notification_default_to', $user_input_values['admin_content_notification_default_to']);
    $config->set('admin_content_notification_email_limit', $user_input_values['admin_content_notification_email_limit']);
