...
namespace Drupal\search_api_sorts_widget\Controller;
...
class AdminController extends ControllerBase {
  ...
  public function displayListing(IndexInterface $search_api_index) {
    ...
    foreach ($this->searchApiDisplayManager->getInstances() as $display) {
      if ($display->getIndex() instanceof IndexInterface && $search_api_index->id() === $display->getIndex()->id()) {
        ...
        $links['configure'] = [
          'title' => $this->t('Manage sorts widget'),
    ...
    $build['table'] = [
      ...
      '#title' => $this->t('Sorts widget configuration.'),
      '#rows' => $rows,
  ...
  public function redirectEditForm(SearchApiSortsWidget $search_api_sorts_widget) {
    ...
    $display_id = $this->getOriginalConfigId($search_api_sorts_widget->getDisplayId());
    /** @var \Drupal\search_api\Display\DisplayInterface $display */
    $display = $this->searchApiDisplayManager->createInstance($display_id);

    return $this->redirect('search_api_sorts_widget.search_api_display.sorts_widget', [
      'search_api_index' => $display->getIndex()->id(),
      'search_api_display' => $search_api_sorts_widget->getDisplayId(),
    ]);
  }
