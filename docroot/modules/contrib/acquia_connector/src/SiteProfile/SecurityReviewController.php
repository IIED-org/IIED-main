  ...
  private function getSecurityReviewRunCheck($module, $check_name, $check, $log, $store = FALSE) {
    ...
    if ($log && !is_null($return['result'])) {
      ...
      if ($check_result['result']) {
        $this->getSecurityReviewLog($module, $check_name, '@name check passed', $variables, RfcLogLevel::INFO);
      }
      else {
        $this->getSecurityReviewLog($module, $check_name, '@name check failed', $variables, RfcLogLevel::ERROR);
      }
    }
    return $check_result;
  }
