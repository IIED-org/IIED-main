...
class NameListFormatListBuilder extends ConfigEntityListBuilder {
   ...
   */
  public function __construct(EntityTypeInterface $entity_type, EntityStorageInterface $storage, NameFormatterInterface $formatter, NameFormatParserInterface $parser, GeneratorInterface $generator) {
  ...
  public function buildRow(EntityInterface $entity) {
    if (!$entity instanceof NameListFormat) {
      throw new \LogicException(sprintf(
        'Expected %s in %s::buildRow(), got %s.',
        NameListFormat::class,
        static::class,
    ...
    $and_options = $this->formatter->getLastDelimiterTypes();
    $and_delimiter = $and_options[$settings['and']] ?? $this->t('-- invalid option --');

    $and_behavior_options = $this->formatter->getLastDelimiterBehaviors(FALSE);
